...
import type { PremadeAtlas } from './assets';
...
const WORKSPACE_CROP_OFFSET = { x: 0, y: 0 };
const WORKSPACE_CABINET = { x: 128, y: 198, w: 20, h: 24 };

export interface PixiWorkspaceScene {
  container: PIXI.Container;
  setState: (state: AppState) => void;
  update: (time: number) => void;
}

export async function createPixiWorkspaceScene(initialState: AppState): Promise<PixiWorkspaceScene> {
  ...
  const [designTexture, uiIconTextures, premadeAtlases] = await Promise.all([
    loadOfficeDesignTexture('design2'),
    loadUiIconTextures(),
    loadPremadeAtlases(),
  ]);
  ...
  if (designTexture) {
    const cropped = cropTexture(
      ...
      IW,
      IH,
      WORKSPACE_CROP_OFFSET.x,
      WORKSPACE_CROP_OFFSET.y,
      16
    );
  ...
  const cabinetEffect = createCabinetToolEffect(actorLayer, WORKSPACE_CABINET, 'FILE_WRITE');
  ...
  let charX = DESIGN_POSITIONS.idle.x;
  let charY = DESIGN_POSITIONS.idle.y;
...
function getTargetPosition(activity: Activity): { x: number; y: number } {
  switch (activity) {
    case 'thinking':
      return DESIGN_POSITIONS.thinking;
    case 'responding':
      return DESIGN_POSITIONS.typing;
    case 'waiting':
      return DESIGN_POSITIONS.waiting;
    default:
      return DESIGN_POSITIONS.idle;
  }
}
